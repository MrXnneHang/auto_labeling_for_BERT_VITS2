...
from utils.util import clean_list, clean_txt, get_file_list, load_config, read_hot_words
...
def cut(wav_name):
    ...
    with open(f"./tmp/final_{wav_name}.txt", "r", encoding="utf-8") as f:
        lines = f.readlines()
    cut_wav(
        wav_filename=f"./raw_audio/{wav_name}.wav",
        lines=lines,
...
def main():
    ...
    for index, name in enumerate(audios):
        os.rename(
            "./raw_audio/" + name,
            "./raw_audio/" + character_name + "_" + str(index) + ".wav",
        )
    file_names = get_file_list("./raw_audio/")
