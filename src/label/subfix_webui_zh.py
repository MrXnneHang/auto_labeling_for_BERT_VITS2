...
def b_change_index(index, batch):
    ...
    for i, _ in enumerate(datas):
        output.append(gr.Textbox(label=f"Text {i + index}", value=_[g_json_key_text]))
    for _ in range(g_batch - len(datas)):
        output.append(gr.Textbox(label=f"Text", value=""))
    for _ in datas:
...
def b_delete_audio(*checkbox_list):
    ...
    #     b_save_file()
    return gr.Slider(
        value=g_index, maximum=(g_max_json_index if g_max_json_index >= 0 else 0)
...
def b_audio_split(audio_breakpoint, *checkbox_list):
    ...
    g_max_json_index = len(g_data_json) - 1
    return gr.Slider(value=g_index, maximum=g_max_json_index), *b_change_index(
        g_index, g_batch
    )
