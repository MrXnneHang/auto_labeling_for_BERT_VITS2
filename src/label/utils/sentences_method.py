def seg_sentences(sentences, cutline):
    ...
    for sentence_index, sentence in enumerate(sentences):
        ...
        for ts_index, start_end in enumerate(sentence["ts_list"]):
            if (
                start_end[0] - latest_end > cutline and latest_end != 0
        ...
        if cut_points != []:
            ...
            for index, cut_point in enumerate(cut_points):
                ...
                if index == 0:  ##第一字句
                    ...
                    new_ts_list = sentence["ts_list"][
                        cut_points[index - 1][1] : cut_point[1]
                    ]
                    new_ts_lists.append(new_ts_list)
                # 最后字句
...
def generate_replacements(sentences, cutline):
    (
        new_texts_in_sentences,
        new_ts_list_in_sentences_,
        new_seg_text_in_sentences,
        all_cut_points_in_sentences,
    ) = seg_sentences(sentences, cutline=cutline)
        ...
        if len(cut_points) != 0:
            ...
            for i in range(len(cut_points) + 1):
                new_sentences.append(
                    {
                        "text": new_texts_in_sentences[sentence_index][i],
                        "ts_list": new_ts_list_in_sentences_[sentence_index][i],
